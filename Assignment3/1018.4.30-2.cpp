...
int main() {
    ...
    for (int i = 1; i < len_tmp; i++) {
        ...
        suf_str[i - 1].print();
        printf("\n");
        int max_pre = StringType::countPrefix(suf_str[i - 1], suf_str[i]);
        printf("%d\n", max_pre);
        if (max_pre > ans_len || (max_pre == ans_len && ans_pos < max(suf_str[i - 1].length(), suf_str[i].length()))) {
            printf("!!!!!!!\n");
            ans_len = max_pre;
            ans_pos = max(suf_str[i - 1].length(), suf_str[i].length());
